import secrets
from ..models import Case, Item, Inventory, User, CasePity, Vault, CaseItem
...
async def open_case(db, user_id, case_id, op_key):
    ...
    rows = (await db.execute(
        select(CaseItem, Item)
        .join(Item, CaseItem.item_id == Item.id)
        .where(CaseItem.case_id == case.id, Item.rarity == rarity)
    )).all()
    if not rows:
        # Backward-compatible fallback for old databases without populated case_items.
        rows = [(None, it) for it in (await db.execute(select(Item).where(Item.rarity == rarity))).scalars().all()]
    if not rows: raise ValueError("Для этой редкости нет предметов")
    total = sum(float(row.weight if row else 1.0) for row, _ in rows)
    pick = secrets.randbelow(max(1, int(total * 1_000_000))) / 1_000_000
    acc = 0.0
    item = rows[-1][1]
    for row, candidate in rows:
        acc += float(row.weight if row else 1.0)
        if pick <= acc:
            item = candidate
            break
