...
async def refresh_energy(db, u):
    ...
    minutes=max(0,int((now-last).total_seconds()//300))
    if minutes>0:
        u.energy=min(100,u.energy+minutes)
        u.energy_updated_at=last+timedelta(minutes=minutes*5)
    return u
...
async def event_claim(event_id:int,request:Request,uid:int=Depends(current_user),db:AsyncSession=Depends(get_db)):
    ...
    db.add(Operation(user_id=uid,key=key,result={"event_id":event_id,"vld":1000}));await db.commit();return {"ok":True,"vld":1000,"xp":100}
...
async def equip_cosmetic(product_id:int,uid:int=Depends(current_user),db:AsyncSession=Depends(get_db)):
    p=await db.get(StarsProduct,product_id);uc=(await db.execute(select(UserCosmetic).where(UserCosmetic.user_id==uid,UserCosmetic.product_id==product_id))).scalar_one_or_none()
...
@app.get("/admin/")
async def admin_page(): return FileResponse("admin/index.html")

@app.get("/style.css")
async def app_css(): return FileResponse("frontend/style.css", media_type="text/css")

@app.get("/app.js")
async def app_js(): return FileResponse("frontend/app.js", media_type="application/javascript")

@app.get("/")
async def app_page(): return FileResponse("frontend/index.html")
