...
async def schema(engine):
    ...
    # Tests must never replace the migrated schema with create_all/drop_all.
    yield
...
async def users(session_factory):
    async with session_factory() as db:
        ...
        a=User(telegram_id=int(token[:9], 16),username=f"qa_a_{token}",nickname="QA A",referral_code=f"ref_QAA_{token}")
        b=User(telegram_id=int(token[1:10], 16),username=f"qa_b_{token}",nickname="QA B",referral_code=f"ref_QAB_{token}")
