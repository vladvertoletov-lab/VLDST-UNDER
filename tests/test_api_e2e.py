...
async def setup(engine):
    # Schema lifecycle belongs to Alembic/release CI, never Base.metadata create/drop.
    yield
...
async def ctx(sf):
    async with sf() as db:
        a=User(telegram_id=70001,nickname="E2E A",referral_code="e2e_a")
        b=User(telegram_id=70002,nickname="E2E B",referral_code="e2e_b")
        ...
        ach=Achievement(code=f"E2E_FIRST_STEP_{suffix}",name="First Step",category="Progress",reward_vld=10,reward_xp=5,requirement={})
